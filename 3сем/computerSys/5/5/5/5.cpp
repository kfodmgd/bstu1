...
void Ping(const char* cHost,//адрес хоста
    unsigned int Timeout,   //время ответа
    unsigned int RequestCount) {//колво запросов

    HANDLE hIP = IcmpCreateFile();
    if (hIP == INVALID_HANDLE_VALUE) {
        WSACleanup();//завершение Winsock 2 DLL
        return;
    }
    ...
    unsigned long ipaddr = inet_addr(cHost);//преобразование адреса
...
int main()
{
    ...
    int ms;
    int reqNum;
    cout << "Введите адрес хоста:";
    cin >> hostIP;
    cout << "Время ожидания ответа в мс:";
    cin >> ms;
    cout << "Количество запросов:";
    cin >> reqNum;
    Ping(hostIP, ms, reqNum);
